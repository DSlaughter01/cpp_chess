...
class Game {
    ...
    private: 
        ...
        uint64_t possibleMoves = BitboardOps::EMPTY_BITBOARD;  
        ...
        // Click variables
        int clickCount = 0;
        int firstClickIdx = INVALID_IDX;
        int secondClickIdx = INVALID_IDX;

        // FEN
        const std::string initialFen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1";
        std::string currentFen = initialFen;
        ...
        uint64_t whiteBitboard, blackBitboard, allPieceBitboard;

        // An array containing all of the bitboards, used for piece lookup
        std::array<uint64_t*, 12> pieceArray {};
        int srcPieceArrayIdx = INVALID_IDX;
        int destPieceArrayIdx = INVALID_IDX;
